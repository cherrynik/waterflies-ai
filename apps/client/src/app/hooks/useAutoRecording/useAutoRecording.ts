...
import { requestMicrophonePermission, stopStreamTracks, handleMicrophoneError } from './utils';
...
export function useAutoRecording({
  ...
  useEffect(() => {
    ...
    if (!initializationStarted.current && !micPermissionGranted && !autoRecordingStarted) {
