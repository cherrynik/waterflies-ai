...
import { createUploadFormData, handleUploadError, logUploadInfo, logProcessingResult, createStandardError } from './utils';
...
export class RecordingUpload {
   ...
   */
  async uploadRecording(recordingData: RecordingData, options?: UploadOptions): Promise<ProcessingResult> {
    if (!recordingData) {
   ...
   */
  async uploadBlob(blob: Blob, options?: UploadOptions): Promise<ProcessingResult> {
