        ...
        <div className="space-y-2 lg:space-y-3 max-h-40 lg:max-h-none overflow-y-auto">
          {actionItems.length > 0 ? (
            actionItems.map((item: string, index: number) => (
              <div
                key={index}
                className="bg-gray-700 p-2 lg:p-3 rounded hover:bg-gray-600 transition-colors duration-300 group"
              >
                <div className="flex items-start space-x-2">
                  <input
                    type="checkbox"
                    className="mt-1 w-3 h-3 lg:w-4 lg:h-4 text-white rounded"
                  />
                  <span className="text-gray-300 text-xs lg:text-sm group-hover:text-white transition-colors duration-200">
                    {item}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-gray-700 p-3 lg:p-4 rounded">
              <p className="text-gray-400 text-xs lg:text-sm italic">
                No specific action items were identified from this conversation.
              </p>
            </div>
          )}
        </div>
