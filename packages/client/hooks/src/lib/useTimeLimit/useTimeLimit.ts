...
const MAX_TIME_SECONDS = 60 as const;
const WARNING_TIME_SECONDS = 45 as const;

export function useTimeLimit({
  maxTimeSeconds = MAX_TIME_SECONDS,
  warningTimeSeconds = WARNING_TIME_SECONDS,
  onTimeLimitWarning,
  onTimeLimitReached,
  ...
  const startTimer = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      setCurrentTime((prevTime) => {
        ...
        if (
          shouldShowWarning(newTime, warningTime, hasTriggeredWarning.current)
        ) {
